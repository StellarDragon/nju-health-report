...
curr_location = ''  # 打卡地点
school_location = '中国江苏省南京市栖霞区九乡河东路'


def fillOneForm(wid, location):
    info = '&IS_TWZC=1&IS_HAS_JKQK=1&JRSKMYS=1&JZRJRSKMYS=1'  # 分别对应四个单选框的值
    link = 'http://ehallapp.nju.edu.cn/xgfw/sys/yqfxmrjkdkappnju/apply/saveApplyInfos.do?WID={wid}&CURR_LOCATION={location}'
    link = link.format(wid=wid, location=location) + info
    driver.get(link)
...
def fillTheForms(res):
    ...
    #     if form['TBZT'] == '0':
    #         if form['TBRQ'] < '2021-01-17' and form['TBRQ'] > '2020-08-10':
    #             print('Date: ' + form['TBRQ'] + '  ' +
    #                   fillOneForm(form['WID'], school_location))
    #         else:
    #             print('Date: ' + form['TBRQ'] + '  ' +
    #                   fillOneForm(form['WID'], curr_location))
